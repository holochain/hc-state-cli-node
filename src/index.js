...
const call_admin_port = async (async_fn, port, args) => {
  try {
    console.log(
      'Invoking call to Admin-Interface at port (%s) and with args (%s)',
      port,
      args ? inspect(args) : '{}'
    )
...
const call_app_port = async (admin_port, async_fn, installedAppId, args) => {
  try {
    console.log(
      'Invoking call to App-Interface for installedAppId (%s) and with args (%s)',
      installedAppId,
      args ? inspect(args) : '{}'
    )

    const appWebsocket = await getAppWebsocket(admin_port, installedAppId)
...
export async function getArgs () {
  ...
  program
    ...
    .action(async (installedAppId) => {
      const result = await call_app_port(
        ...
        appInfo,
        installedAppId,
        installedAppId // sending the id as the appInfo fn payload here
      )
  ...
  program
    .command(
      'zomeCall <InstalledAppId> <DnaHash> <AgentHash> <ZomeName> <ZomeFunction> <Payload>'
    )
    .alias('z')
    .description(
      'call zome function for cell: calls callZome(cell_id, zome_name, fn_name, payload, provenence) -> ZomeCallResult: any'
    )
    .action(async (InstalledAppId, DnaHash, AgentHash, ZomeName, ZomeFunction, Payload) => {
      let payload = null
      ...
      const result = await call_app_port(program.opts().adminPort, zomeCall, InstalledAppId, args)
      console.log('\nZome Call Result :')
